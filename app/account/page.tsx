...
export default function AccountPage() {
  ...
  // This function fetches the user's profile data (e.g., marketing consent)
  const getProfile = useCallback(async (currentSession: Session) => {
    const { data, error } = await supabase
      .from('profiles')
      .select('marketing_consent')
      .eq('id', currentSession.user.id)
      .single(); // .single() expects exactly one row, which is perfect for profiles
  ...
  // This effect runs once to get the user's session and then their profile
  useEffect(() => {
    const getSession = async () => {
      ...
      if (!data.session) {
        ...
        await getProfile(data.session); // Fetch profile data after getting session
        setIsLoading(false);
      }
    };
    getSession();
  ...
  // Manual Account Deletion Handler
  ...
  return (
    <main className="flex flex-col items-center min-h-screen bg-gray-50 p-4 pt-10">
      <div className="w-full max-w-lg bg-white p-8 rounded-xl shadow-lg space-y-8">
        ...
        {/* Future "Points" Section Placeholder */}
        <div>
            <h2 className="text-xl font-semibold text-gray-700">My Points</h2>
            <div className="mt-2 p-4 bg-indigo-50 rounded-md text-gray-900">
                <p>You currently have: <span className="font-bold text-indigo-600">0 Points</span></p>
                <p className="text-xs text-gray-500 mt-1">This feature is coming soon! Use the app daily to earn points.</p>
            </div>
        </div>

        {/* Danger Zone for Manual Deletion */}
